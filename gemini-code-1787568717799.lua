-- ============================================
-- @set9p | SILENT AIM (360°) + ESP + ANTI-AIM + TRIGGERBOT (FOV)
-- ============================================
...
local Settings = {
    ...
    -- Триггербот (Автофаер по целям в FOV с проверкой Mouse.Target)
    TriggerBot = false,
    TriggerDelay = 0,
    ...
    for _, v in pairs(plrs) do
        ...
        if Settings.Aim360 then
            if myHRP then
                ...
                if worldDist <= Settings.AimFOV * 2 then
                    table.insert(validTargets, targetHitPart)
                end
            end
        else
            local screenPos, onScreen = camera:WorldToViewportPoint(targetHitPart.Position)
            if onScreen then
...
-- ============================
-- ТРИГГЕРБОТ (В ПАРЕ С FOV + РАБОЧИЙ КЛИК)
-- ============================
    ...
    while true do
        ...
        if Settings.TriggerBot then
            ...
            if targetPart and targetPart.Parent then
                local character = targetPart.Parent:FindFirstAncestorOfClass("Model")
                if character then
                    ...
                    if player and isEnemy(player) then
                        ...
                        if humanoid and humanoid.Health > 0 then
                            local hitPart = nil
                            if Settings.AimPart == "Torso" then
                                hitPart = character:FindFirstChild("UpperTorso") or character:FindFirstChild("Torso") or character:FindFirstChild("HumanoidRootPart")
                            else
                                hitPart = character:FindFirstChild("Head") or character:FindFirstChild("HumanoidRootPart")
                            end
                            ...
                            if hitPart and hitPart:IsA("BasePart") then
                                ...
                                if onScreen then
                                    local distance = (Vector2.new(screenPos.X, screenPos.Y) - center).Magnitude
                                    if Settings.Aim360 or distance <= Settings.AimFOV then
                                        if not Settings.VisibleCheck or isVisible(hitPart) then
                                            shouldShoot = true
                                        end
                                    end
                                end
            ...
            if shouldShoot then
                if Settings.TriggerDelay > 0 then
...
CombatTab:CreateSection("Триггербот (Автофаер в FOV)")

CombatTab:CreateToggle({
    Name = "Включить TriggerBot",
    CurrentValue = false,
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Триггербот успешно обновлен и работает!",
    Duration = 3,
    Image = 4483362458,
})