-- ============================================
-- @set9p | SILENT AIM (360°) + ESP + ANTI-AIM + AUTO-FIRE
-- ============================================
...
RunService.RenderStepped:Connect(function()
    ...
    -- Логика Автофаера
    if Settings.AutoFire and target and target:IsA("BasePart") then
        local now = tick()
        if now - lastAutoFireTime >= Settings.AutoFireDelay then
            lastAutoFireTime = now
            pcall(function()
                local center = Camera.ViewportSize / 2
                VirtualInputManager:SendMouseButtonEvent(center.X, center.Y, 0, true, game, false)
                task.wait()
                VirtualInputManager:SendMouseButtonEvent(center.X, center.Y, 0, false, game, false)
            end)
        end
    end
end)
...
CombatTab:CreateSection("Автофаер (Авто-огонь)")

CombatTab:CreateToggle({
    Name = "Автофаер по цели",
    CurrentValue = false,
    Flag = "AutoFireToggle",
...
CombatTab:CreateSlider({
    Name = "Задержка автофаера",
    Range = {0.02, 0.5},
    Increment = 0.01,
    Suffix = "s",
    CurrentValue = 0.05,
    Flag = "AutoFireDelaySlider",
    Callback = function(v) Settings.AutoFireDelay = v end
})
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Автофаер успешно добавлен!",
    Duration = 3,
    Image = 4483362458,
})