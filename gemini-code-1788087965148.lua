...
-- Проверка конкретной преграды на простреливаемость
local function isPenetrable(hitPart)
    if not hitPart then return false end
    
    -- 1. Проверка через атрибуты
    if hitPart:GetAttribute("Penetrable") == true or hitPart:GetAttribute("Wallbang") == true then
        return true
    end
    
    -- 2. Проверка через теги CollectionService
    if CollectionService:HasTag(hitPart, "PenetrableWall") then
        return true
    end
    
    -- 3. Проверка по материалу или прозрачности (например, стекло или полупрозрачные объекты)
...
-- Продвинутая функция проверки видимости с поиском простреливаемых стен и ограничением дистанции
local function canPenetrateShoot(targetPart)
    ...
    -- Ограничение по максимальной дистанции (чтобы не стреляло через всю карту)
