-- ============================================
-- @set9p | SILENT AIM + AUTO TRIGGER + SOUND & TRAIL COOLDOWN + ESP + BHOP BOOST
-- ============================================
...
local bhopBoostSpeed = 25 -- Сила ускорения при прыжке
            ...
            if char then
                ...
                if humanoid and humanoid.Health > 0 and hrp then
                    if humanoid.FloorMaterial ~= Enum.Material.Air then
                        ...
                        if bhopBoostSpeed > 0 then
                            local cam = Workspace.CurrentCamera
                            if cam then
                                local lookVect = cam.CFrame.LookVector
                                local moveDir = Vector3.new(lookVect.X, 0, lookVect.Z).Unit
                                
                                local currentVel = hrp.AssemblyLinearVelocity
                                hrp.AssemblyLinearVelocity = Vector3.new(
                                    moveDir.X * (humanoid.WalkSpeed + bhopBoostSpeed),
                                    currentVel.Y,
                                    moveDir.Z * (humanoid.WalkSpeed + bhopBoostSpeed)
                                )
                            end
                        end
                    end
...
MiscTab:CreateSlider({
    Name = "Скорость Bhop (Ускорение)",
    Range = {0, 100},
    Increment = 5,
    Suffix = " studs",
    CurrentValue = 25,
    Flag = "BhopBoostSlider",
    Callback = function(v) bhopBoostSpeed = v end
})
