-- ============================================
-- @set9p | SILENT AIM + WALLBANG (ITERATIVE RAYCAST) + AUTO TRIGGER + ESP + BHOP
-- ============================================
...
-- Проверка материала для прострела (Wallbang / Iterative Raycasting)
local function isPenetrableMaterial(part)
    ...
    if part.Name:lower():find("wood") or part.Name:lower():find("glass") then
        return true
    end
    return false
end

-- Продвинутая функция проверки видимости с прострелом сквозь стекла и тонкие преграды
local function canPenetrateShoot(targetPart)
    ...
    local ignoreList = {char, targetPart.Parent}
    local maxPasses = 4 
    ...
    for i = 1, maxPasses do
        ...
        if isPenetrableMaterial(hitPart) then
            table.insert(ignoreList, hitPart)
            local distanceCovered = (result.Position - currentOrigin).Magnitude
            local totalLength = direction.Magnitude
            local remainingDist = totalLength - distanceCovered
            
            if remainingDist <= 0 then
                break
            end
            
            currentOrigin = result.Position + (currentDirection.Unit * 0.1)
            currentDirection = direction.Unit * remainingDist
        else
            return false
        end
    end
...
local function GetClosestPlayer()
    ...
    for _, v in pairs(plrs) do
        ...
        if Settings.VisibleCheck and not canPenetrateShoot(targetHitPart) then continue end
...
RunService.RenderStepped:Connect(function()
    ...
    if lockedTarget and lockedTarget.Parent then
        ...
        if not hum or hum.Health <= 0 or (Settings.VisibleCheck and not canPenetrateShoot(lockedTarget)) then
            lockedTarget = nil
        end
    else
        lockedTarget = nil
...
CombatTab:CreateToggle({
    Name = "Visible Check (С прострелом стекол)",
    CurrentValue = true,
